...
def circle_overlaps_collision_grid(grid: CollisionGrid, x: float, y: float, r: float, n_samples: int=16) -> bool:
...
def collide_ball_bitmap(x: float, y: float, vx: float, vy: float, r: float, grid: CollisionGrid, iterations: int=4, rim_samples: int=16, max_separate_steps: int=96) -> tuple[float, float, float, float]:
...
def resolve_circle_rect(x: float, y: float, vx: float, vy: float, r: float, rect: pygame.Rect) -> tuple[float, float, float, float, bool]:
...
def collide_ball_all_rects(x: float, y: float, vx: float, vy: float, r: float, rects: list[pygame.Rect], iterations: int=3) -> tuple[float, float, float, float]:
    for _ in range(iterations):
        hit_any = False
        for rect in rects:
